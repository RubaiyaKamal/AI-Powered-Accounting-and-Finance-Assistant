...
- [ ] T002 Add `ACCOUNT_CODING_CONFIDENCE_THRESHOLD` env var handling (default `0.8`) in `backend/src/config.py` and `backend/.env.example`, per `research.md`'s Confidence Threshold Configuration decision
- [ ] T003 [P] Create the `Account` SQLAlchemy model (`id`, `code` unique, `name` unique, `type` enum, `is_custom`) in `backend/src/models/account.py`, per `data-model.md`
- [ ] T004 [P] Create the `AccountCoding` SQLAlchemy model (`id`, `expense_entry_id` FK unique, `account_id` FK, `confidence_score`, `source`, `status`) in `backend/src/models/account_coding.py`, per `data-model.md`
- [ ] T005 [P] Create the `JournalEntry` SQLAlchemy model (`id`, `expense_entry_id` FK, `account_coding_id` FK, `debit_account_id` FK, `credit_account_id` FK, `amount`, `date`, `status`, `reverses_journal_entry_id` self-referencing FK) in `backend/src/models/journal_entry.py`, per `data-model.md`
- [ ] T006 Write the Alembic migration creating the three tables from T003–T005 and seeding the starter chart of accounts (one `Cash` asset account, plus one expense account per existing seeded category: Utilities, Rent, Salaries, Supplies) in `backend/migrations/versions/`
- [ ] T007 [P] Create `Account` Pydantic request/response schemas in `backend/src/schemas/account.py`, per `contracts/ledger-api.md`
- [ ] T008 [P] Create `AccountCoding` Pydantic response schemas (including a nested `account` and, when posted, a nested `journal_entry`) in `backend/src/schemas/account_coding.py`, per `contracts/ledger-api.md`
- [ ] T009 [P] Create `JournalEntry` Pydantic response schemas in `backend/src/schemas/journal_entry.py`, per `contracts/ledger-api.md`
...
- [ ] T010 [US1] Implement `AccountService.list_accounts` and `AccountService.create_account` (rejects a case-insensitive duplicate code or name, FR-002) in `backend/src/services/account_service.py`
- [ ] T011 [US1] Implement `GET /api/accounts` and `POST /api/accounts` in `backend/src/api/accounts.py`, per `contracts/ledger-api.md`
- [ ] T012 [US1] Implement the `suggest_account_coding` agent tool (description + existing accounts → suggested account name + confidence score) in `backend/src/agent/ledger_tools.py`, per FR-003, mirroring `expense_tools.py`'s `suggest_category`
- [ ] T013 [US1] Implement `LedgerService.post_journal_entry` (constructs a balanced debit/credit pair from an `AccountCoding` and its `ExpenseEntry`'s amount/date; refuses to post if the debit and credit accounts would be equal or either account no longer exists) in `backend/src/services/ledger_service.py`, per FR-007–FR-008, FR-015
- [ ] T014 [US1] Implement `LedgerService.reverse_journal_entry` (creates a new `JournalEntry` with debit/credit accounts swapped, same amount, `reverses_journal_entry_id` set; marks the original `status=reversed`) in `backend/src/services/ledger_service.py`, per `research.md`'s Reversal Mechanics decision (depends on T013)
- [ ] T015 [US1] Implement `LedgerService.suggest_coding` (calls `suggest_account_coding`, resolves the suggested name to an `Account`, creates an `AccountCoding` row; auto-approves and calls `post_journal_entry` when confidence is at/above the configured threshold, else leaves `status=pending_review`) in `backend/src/services/ledger_service.py`, per FR-003–FR-005 (depends on T012, T013)
- [ ] T016 [US1] Implement `LedgerService.approve_coding` (`pending_review` → `approved`, calls `post_journal_entry`) in `backend/src/services/ledger_service.py`, per US1 acceptance scenario 2 (depends on T013)
- [ ] T017 [US1] Implement `LedgerService.correct_coding` (sets `account_id` + `source=user` + `status=approved`; if a `posted` `JournalEntry` already exists for this coding, calls `reverse_journal_entry` then `post_journal_entry` for the corrected account) in `backend/src/services/ledger_service.py`, per FR-006, FR-011 (depends on T013, T014)
- [ ] T018 [US1] Implement `POST /api/expenses/{expense_id}/coding/suggest`, `GET /api/expenses/{expense_id}/coding`, `POST /api/expenses/{expense_id}/coding/approve`, and `PATCH /api/expenses/{expense_id}/coding` in `backend/src/api/ledger.py`, per `contracts/ledger-api.md`
- [ ] T019 [US1] Register the accounts and ledger routers in `backend/src/main.py`
- [ ] T020 [US1] Build the `AccountCoding` component (shows the suggested account + confidence score or a pending-review state, an approve control, and an account picker to correct) in `frontend/src/components/AccountCoding.tsx`
- [ ] T021 [US1] Wire `AccountCoding` into the expense list/detail view — calling `suggest` the first time an entry's coding is opened, and `approve`/`correct` on user action — in `frontend/src/services/ledgerApi.ts` and `frontend/src/components/ExpenseList.tsx`
...
- [ ] T022 [US2] Implement `GET /api/journal-entries/{id}` (single entry detail, including `reverses_journal_entry_id` linkage) in `backend/src/api/ledger.py`, per `contracts/ledger-api.md`
- [ ] T023 [US2] Implement `LedgerService.reverse_journal_entry_for_expense` and wire it into the existing `DELETE /api/expenses/{id}` handler in `backend/src/api/expenses.py`, per FR-012 — the one integration touch-point into already-shipped `001-expense-entry` code called out in `plan.md`
- [ ] T024 [US2] Add a defensive check in `LedgerService.post_journal_entry` that the constructed debit and credit amounts are always equal and that neither account has been deleted, rather than relying solely on DB foreign-key constraints, in `backend/src/services/ledger_service.py`, per FR-008, FR-015
...
- [ ] T025 [US3] Implement `LedgerService.list_journal_entries` with `date_from`/`date_to`/`account_id` filters (matching either the debit or credit account) in `backend/src/services/ledger_service.py`, per FR-009
- [ ] T026 [US3] Implement `GET /api/journal-entries` (list + filters) in `backend/src/api/ledger.py`, per `contracts/ledger-api.md`
- [ ] T027 [US3] Validate `date_from <= date_to` in the list endpoint, returning `422` otherwise, in `backend/src/api/ledger.py` — mirrors `001-expense-entry`'s expense-list validation
- [ ] T028 [US3] Build the `JournalEntryList` component with account/date-range filters and a link back to each entry's source expense entry in `frontend/src/components/JournalEntryList.tsx`
- [ ] T029 [US3] Build the ledger page wiring `JournalEntryList` in `frontend/src/app/ledger/page.tsx`
...
- [ ] T030 [P] Update the workflow diagram (`docs/workflow-diagram.drawio`) to include the `suggest_account_coding` tool and the auto-posting flow, and confirm the shareable URL in `.specify/memory/constitution.md`/`README.md` still reflects the current system — required before this feature's PR merges, per the Constitution Check in `plan.md` (Principle V)
- [ ] T031 [P] Update `README.md` to document the new `ACCOUNT_CODING_CONFIDENCE_THRESHOLD` environment variable
- [ ] T032 Run the `quickstart.md` validation flow end-to-end and fix any gaps found
- [ ] T033 [P] Code cleanup pass across `backend/` and `frontend/` for this feature
